...
I am running [a backend server](http://printer.gofreerange.com), which our office printer is connected to.
...
If you want to make a printer, [here are the components you need](http://printer.gofreerange.com/getting-a-printer), and here is [how to plug it all together](http://github.com/freerange/printer/blob/master/HARDWARE.md).

We are going to make a [small number of printers available to loan](/printer#loan) to people who have an idea they'd like to explore. We'd love to help, and everyone will benefit from the services and content you make.

If you're interested in buying [all the bits you need, pre-configured and ready to go](/printer#kits), you can [let us know here](/printer#kit) (we don't know if we will do that, but if there's enough interest then we'll investigate how).
...
It's also worth mentioning some "historical" printers of note. The printer that ran [Timmy Printface][Timmy] was used by [Tom Taylor](http://tomtaylor.co.uk) in 2009 to [print out daily memos](http://tomtaylor.co.uk/projects/microprinter). His software is perhaps closer in architecture to what I've built (a printer polling from a queue). [Chris Wood](http://flipbit.co.uk/) extended the software to [print tube information](http://flipbit.co.uk/microprinting.html). There's even a [wiki](http://microprinter.pbworks.com/) that has sprung up around the CBM 231 printers that people commonly used.

What most clearly distinguishes this project from previous works is the ease with which independent people can create great-looking quality content for these printers. Rather than being limited to text-based output, far richer designs can be easily constructed using standard web tools like HTML, CSS and Javascript.

And, of course, there is the upcoming [Little Printer](http://bergcloud.com/littleprinter) from [BERG](http://berglondon.com), available some time this year. While it's clear that I've been interested in "whimsical thermal printing" for some time ([Timmy][] was built a year ago, long before Little Printer was announced), it's obvious that the open source project I am presenting here shares some common aspects with [BERG](http://berglondon.com)'s own approach.
...
With the open source software and hardware outlined above, we now have some tools to collaboratively explore the coastlines and inland jungles of this new territory.
...
* [Send a message to our printer](http://wee-mail.herokuapp.com/send/lazyatom)
