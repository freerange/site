...
<img src="<%= image_path('blog/free-agent-explaining-vat-transaction.png') %>" width="100%" alt="FreeAgent automatically determining the VAT for a transaction">

This will mean that our accounts include some VAT -- 20% of £7.69 in this case -- that we never actually paid:

<img src="<%= image_path('blog/free-agent-us-transaction-with-vat.png') %>" width="100%" alt="VAT for a transaction that shouldn't exist">
...
Stalwart solver-of-problems [James M](/james-mead) took this observation, along with the [FreeAgent API][] (and a [small wrapper that we wrote][freeagent-wrapper]) to write a simple script, which [Chris](/chris-roos) then massaged into very simple application that we call *[FreeAgent Sanity Check][]*.
