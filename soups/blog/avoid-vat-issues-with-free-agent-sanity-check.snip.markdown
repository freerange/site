...
Stalwart solver-of-problems {l chris-roos,Chris} took this observation, along with the [FreeAgent API][] (and a [small wrapper that we wrote][freeagent-wrapper]) to write a very simple application that we call *[FreeAgent Sanity Check][]*.
